...
struct AgeCalculator {
    static func calculate(for person: Person, at date: Date) -> ExactAge {
        ...
        if calendar.isDate(startOfDayDate, equalTo: startOfDayBirthDate, toGranularity: .day) {
            return ExactAge(
                years: 0, months: 0, days: 0, isPregnancy: false, pregnancyWeeks: 0, isNewborn: true
            )
        }

        if startOfDayDate < startOfDayBirthDate {
            let pregnancyStart =
                calendar.date(byAdding: .month, value: -9, to: startOfDayBirthDate)
                ?? startOfDayBirthDate
            let components = calendar.dateComponents(
                [.day], from: pregnancyStart, to: startOfDayDate)
            let daysFromPregnancyStart = components.day ?? 0
            let weeksPregnant = min(40, (daysFromPregnancyStart / 7) + 1)
            return ExactAge(
                years: 0, months: 0, days: 0, isPregnancy: true, pregnancyWeeks: weeksPregnant,
                isNewborn: false)
        }

        let components = calendar.dateComponents(
            [.year, .month, .day], from: startOfDayBirthDate, to: startOfDayDate)
    ...
    static func sectionForPhoto(_ photo: Photo, person: Person) -> String {
        ...
        let yearStart = calendar.date(
            from: DateComponents(
                year: calendar.component(.year, from: photo.dateTaken),
                month: calendar.component(.month, from: person.dateOfBirth),
                day: calendar.component(.day, from: person.dateOfBirth)))!
        let ageAtYearStart = calendar.dateComponents(
            [.year], from: person.dateOfBirth, to: yearStart
        ).year!
